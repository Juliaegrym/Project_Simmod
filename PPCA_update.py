...
def attack(place, i, j, grid):
    ...
    if (place.kind == 1) and (low_pred > 0):
        ...
        if survive_prob < r:
            old_veg = place.veg
            place = Cell(0)
            place.veg = old_veg  # Vegetation stays the same
            place.state = 3
    ...
    # Cell contains a low level predator
        ...
        if (1 - ((1 - Cell(1).catchprob) ** prey) >= r) and (place.kind == 2) and (prey > 0):
            ...
            old_veg = grid[prey_indices[remove_index][0]][prey_indices[remove_index][1]].veg
            grid[prey_indices[remove_index][0]][prey_indices[remove_index][1]] = Cell(0)
            grid[prey_indices[remove_index][0]][prey_indices[remove_index][1]].veg = old_veg
    ...
    # Cell contains top level predator
        ...
        if (1 - (1 - Cell(2).catchprob) ** low_pred) >= r:
            ...
            # Changes a random prey grid cell in neighbourhood to an empty cell with state = 3
            old_veg = grid[low_pred_indices[remove_index][0]][low_pred_indices[remove_index][1]].veg
            grid[low_pred_indices[remove_index][0]][low_pred_indices[remove_index][1]] = Cell(0)
            grid[low_pred_indices[remove_index][0]][low_pred_indices[remove_index][1]].veg = old_veg
...
def update(dim, grid):
    ...
    for k in range(dim):
        for l in range(dim):
            ...
            new_grid[k][l] = state
    # Vegetation update
    new_grid = update_veg(new_grid)
    # Health update
    new_grid = update_hp(new_grid)
...
def update_veg(grid):
    ...
    for i in range(dim):
        for j in range(dim):
            place = grid[i][j]
            place.veg += dVeg[place.kind]
            if place.kind == 1 and place.veg > vegC:
                place.hunger += eatHu[1]
    return grid


def update_hp(grid):
    # Updates health based on hunger
    # Also takes into consideration if animal dies of poor health
    ...
    for i in range(dim):
        for j in range(dim):
            place = grid[i][j]
            if place.kind != 0:
                place.health += coeffHpDmg*(place.hunger - 50)    # Decreases/increases health proportional to hunger

                if place.health <= 0:
                    old_veg = place.veg
                    grid[i][j] = Cell(0)                        # Clears cell (animal dies) because of poor health
                    # Should vegetation increase (death ---> soil etc.)??
                    grid[i][j].veg = old_veg                    # Vegetation stays the same
